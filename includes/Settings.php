...
class Settings
{
	...
	{
		$config = get_option('wooms_config', []);
		if (empty($key)) {
			return $config;
		}
		return $config[$key] ?? null;
	...
	public static function setValue($key, $value)
	{
		$config = get_option('wooms_config', []);
		$config[$key] = $value;
		return update_option('wooms_config', $config, false);
	}
	...
	{
		return sprintf('wooms_config[%s]', $key);
	}
	...
	public static function display_wooms_replace_title()
	{
		...
		?>
		<p>
			<small>Если включить опцию, то плагин будет обновлять заголовки продукта из МойСклад. Иначе при наличии заголовка
