...
= 9.15 =
TBD
- Улучшено: обновлены требования и совместимость плагина для актуальных версий WordPress и WooCommerce.
- Улучшено: мониторинг синхронизации товаров переведен на recurring-задачу Action Scheduler вместо обычного WP-Cron события.
- Улучшено: управление фоновым daemon-процессом Action Scheduler стало надежнее, включая автозапуск wp-env, PID-файл и корректную остановку процесса.
- Исправлено: повторная постановка в очередь изображений вариаций больше не выполняется, если у вариации уже установлена актуальная миниатюра из МойСклад.
- Исправлено: некорректные задачи загрузки изображений вариаций теперь очищаются безопасно, а image_id сохраняется только после успешной загрузки файла.
- Улучшено: настройки синхронизации продуктов, изображений и использования кода как артикула перенесены в более подходящий раздел страницы настроек.
- Улучшено: добавлены автотесты для обработки изображений товаров и вариаций.
...
- Исправлено: критическая ошибка перезаписи переменных в цикле обработки атрибутов, приводившая к потере данных при наличии нескольких доп. полей у товара.
- Исправлено: не работала ручная синхронизация отдельного товара из-за опечатки в ключе запроса (single).
...
- Улучшено: информативность журналов (логов) синхронизации атрибутов в WooCommerce Status Logs.
...
- Исправлены опечатки в интерфейсе настроек и системных сообщениях.
