...
class WooMS_Product_Import_Walker
{
    ...
    function notice_results()
    {
      ...
      ?>
      <div class="updated">
        <p><strong>Успешное завершение импорта</strong></p>
        <?php
      ...
      switch ($action) {
        case 'wooms_products_start_import':
          ...
          $args =[
            ...
            'batch' => '1',
            '_wpnonce' => wp_create_nonce('wooms_nonce_walker')
          ];
  ...
  */
    function walker()
    {
        ...
        if (empty(get_option('wooms_check_security_disable'))) {
            check_ajax_referer( 'wooms_nonce_walker', '_wpnonce' );
        }
        ...
        try {
            ...
            if (isset($_GET['batch'])) {
                $args = [
                  'action' => 'wooms_walker_import',
                  '_wpnonce' => wp_create_nonce('wooms-nonce'),
                  'batch' => 1,
                  'count' => $iteration,
